# DONIA_PACK_TN_INFO_CALL_EDU_1SHOT.ps1  (PowerShell 5.1)
...
# server: /health, /api/info?category=...&max=25
WriteText $proxySrv @"
...
# -------------------------
# 3) Create Tunisia info pages (Culture/Sport/Politics/Tunisia)
# -------------------------
$infoPages = @(
...
export default function Page() {
  ...
  return (
    <div style={{ padding: 16, maxWidth: 1100, margin: "0 auto" }}>
      <h1 style={{ fontSize: 26, fontWeight: 800, marginBottom: 6 }}>${($x.title)}</h1>
...
export type SignalPayload = { room: string; data: any };
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      pc.onicecandidate = (ev) => {
        if (ev.candidate) {
          socket.emit("signal", { room, data: { type: "ice", candidate: ev.candidate } });
        }
      };
    ...
    <div style={{ padding: 16, maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div style={{ border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, padding: 12 }}>
          ...
          <video ref={localVideoRef} autoPlay muted playsInline controls={false} style={{ width: "100%", borderRadius: 12, background: "black", minHeight: 220 }} />
        </div>

        <div style={{ border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, padding: 12 }}>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Distant</div>
          <video ref={remoteVideoRef} autoPlay playsInline controls={false} style={{ width: "100%", borderRadius: 12, background: "black", minHeight: 220 }} />
        </div>
      </div>

      <div style={{ marginTop: 12, fontSize: 12, opacity: 0.75 }}>
        Note: STUN public par défaut. TURN à ajouter plus tard pour réseaux difficiles.
      </div>
    </div>
  );
}
...
# -------------------------
# 5) Education Tunisia Generator (exams + plans + lessons)
# -------------------------
...
## Contenu (template)
> Ce contenu est un **fallback local**. Configure VITE_AI_GATEWAY_URL pour génération IA.
...
### Traçabilité
- Programme: Tunisie (à préciser: année/système)
...
Type évaluation: ${input.typeEval || "sommative"}
Format: ${input.format || "mix"}
Objectifs: ${input.objectifs || ""}

Contraintes:
- Sortie en Markdown structuré.
- Sections standardisées (Consignes générales, Compétences évaluées, Critères de réussite, Barème, Traçabilité).
- Barème total cohérent (ex: /20).
- Progression graduée.
- Ne copie pas mot pour mot des sources.
- Ajouter une section Traçabilité.
`;
...
# -------------------------
# 6) Patch App.tsx routes + imports + add call buttons hook (optional)
# -------------------------
$appPath = Join-Path $Root "src\App.tsx"
if(!(Test-Path $appPath)){ Die "src\App.tsx not found. You said src/App.tsx contains <Routes> but file not found." }
...
foreach($line in $importBlock -split "`r?`n"){
  if($line.Trim().Length -eq 0){ continue }
  if($app -notmatch [regex]::Escape($line)){
    # insert after last import line
    $app = [regex]::Replace($app, "(^import .*?;(\r?\n))+",
      { param($m) $m.Value + $line + "`r`n" }, 1)
  }
}

# Routes to inject before </Routes>
$routeBlock = @"
...
# -------------------------
# 7) Add call buttons into chat page (best-effort)
#   - Search a likely chat page: contains "Room" + "Messages" or "send"
#   - If found: inject 2 buttons near top
# -------------------------
Info "Searching chat page to inject call buttons (best-effort)..."
$chatCandidates = Get-ChildItem -Recurse -File (Join-Path $Root "src") -ErrorAction SilentlyContinue |
  Where-Object { $_.Extension -in ".tsx",".jsx" } |
  Select-Object -ExpandProperty FullName

$chatFile = $null
foreach($f in $chatCandidates){
  $t = ReadText $f
  if(!$t){ continue }
  if($t -match "Room" -and $t -match "message" -and $t -match "<button"){ $chatFile = $f; break }
}
if($chatFile){
  BackupFile $chatFile
  $t = ReadText $chatFile

  if($t -notmatch "Appel audio"){
    # Inject buttons after first heading/div with title (simple heuristic)
    $btns = @"
<div style={{display:"flex", gap:8, margin:"8px 0"}}>
  <button onClick={() => window.location.href = \`/social/call?room=\${roomId || "class-1"}&mode=audio\`}>Appel audio</button>
  <button onClick={() => window.location.href = \`/social/call?room=\${roomId || "class-1"}&mode=video\`}>Appel vidéo</button>
</div>
"@

    # Ensure roomId exists: if not, create a const roomId = "class-1"
    if($t -notmatch "roomId"){
      $t = $t -replace "(function\s+\w+\s*\(\)\s*\{\s*)", "`$1`r`n  const roomId = ""class-1"";`r`n"
    }

    # insert after first return (
    if($t -match "return\s*\("){
      $t = $t -replace "return\s*\(", ("return (`r`n" + $btns), 1
    } else {
      $t = $btns + "`r`n" + $t
    }

    WriteText $chatFile $t
    Ok "Injected call buttons into chat page: $chatFile"
  } else {
    Ok "Chat page already has call buttons"
  }
} else {
  Warn "Chat page not auto-detected. Use /social/call?room=class-1&mode=audio manually from UI."
}

# -------------------------
# 8) Done + run instructions
# -------------------------
...
Notes:
- WebRTC needs signaling server at VITE_SIGNALING_URL (Socket.IO join/signal).
- If you already have a signaling server, keep it and just set VITE_SIGNALING_URL accordingly.
"@
Ok "Wrote docs/DONIA_TN_PACK_NEXT_STEPS.md"

Ok "DONE ✅ Tunisia Info + WebRTC Call UI + Education TN Generator added."
